...
public class Producer{
    ...
    public void sendMsg1(MQModel model) {
        logger.info("producer:{}", JSON.toJSONString(model));
        rabbitStreamAPI.Producer().send(MessageBuilder.withPayload(model).setHeader("flag","1").build());
    }

    public void sendMsg2(MQModel model) {
        logger.info("producer:{}", JSON.toJSONString(model));
        rabbitStreamAPI.Producer().send(MessageBuilder.withPayload(model).setHeader("flag","2").build());
    }
}
